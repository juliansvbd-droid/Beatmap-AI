...
from beatmap_ai.critic_data import resolve_human_osu_text
...
def main() -> int:
    ...
    with Path(args.manifest).open("r", encoding="utf-8") as f:
        for line in f:
            ...
            negative_map = parse_osu(negative_path.read_text(encoding="utf-8"))
            ...
            pair_bad = len(human_objects) != len(negative_objects)
            if not pair_bad:
                for human_obj, negative_obj in zip(human_objects, negative_objects):
                    if (human_obj.kind != negative_obj.kind
                            or abs(human_obj.time - negative_obj.time) > 0.01
                            or human_obj.slides != negative_obj.slides
                            or abs(human_map.end_time(human_obj) - negative_map.end_time(negative_obj)) > 2.0):
                        pair_bad = True
                        break
            if human_map.slider_multiplier != negative_map.slider_multiplier:
                pair_bad = True
            if [tp.to_line() for tp in human_map.timing_points] != [tp.to_line() for tp in negative_map.timing_points]:
                pair_bad = True
    ...
    print(f"Pairs checked: {len(rows)}; missing files/sources: {missing}; rhythm/timing mismatches: {mismatches}")
